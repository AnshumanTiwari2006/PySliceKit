...
#         plt.savefig(os.path.join(out_dir, "04_paper_figure.png"), dpi=500)
#         plt.savefig(os.path.join(out_dir, "04_paper_figure.pdf"))  # vector version -- use this one in the paper
#         plt.close()
...
#     plt.savefig("./verification_output/00_combined_summary.png", dpi=500)
#     plt.savefig("./verification_output/00_combined_summary.pdf")  # vector version -- use this one in the paper
#     plt.close()
...
def main():
    ...
    for exp in experiments:
        ...
        # Part 4: Charting
        # Single large panel only -- the bar-count comparison (before/BH/Bonferroni)
        # already appears once per dataset in the combined summary figure, so it
        # is dropped here to avoid duplicating it and to let this panel be much
        # bigger and sharper for close zooming.
        fig, ax1 = plt.subplots(1, 1, figsize=(16, 9))
        fig.suptitle(f"{name} — correction verification (m={m} tests, alpha=0.05)", fontsize=22, y=0.98)
        ...
        # To avoid log(0) issues, cap minimum strictly positive
        safe_p = [max(p, 1e-6) for p in raw_p_values]
        ax1.scatter(safe_p, range(len(safe_p)), c=colors, s=180, alpha=0.85, edgecolor='k', linewidth=1.0)
        ax1.set_xscale('log')
        ax1.set_xlabel('Raw p-value (log scale)', fontsize=18)
        ax1.set_ylabel('Segment index', fontsize=18)
        ax1.set_yticks([])
        ax1.tick_params(axis='x', labelsize=15)
        ax1.axvline(0.05, color='gray', linestyle='--', alpha=0.7, linewidth=2, label=r'$\alpha=0.05$')

        bonf_thresh = 0.05 / m
        ax1.axvline(bonf_thresh, color='red', linestyle=':', alpha=0.7, linewidth=2, label=f'Bonf ({bonf_thresh:.5f})')
        ...
        # Still compute the significance counts -- needed for the combined summary figure
        sig_before = sum(1 for r in eligible if r.is_significant)
        sig_bh = sum(manual_bh)
        sig_bonf = sum(manual_bonf)
        ...
        plt.figtext(0.5, 0.01, f"Independent manual BH recomputation: {m}/{m} segments matched library output", ha="center", fontsize=15, style='italic')
        plt.tight_layout(rect=[0, 0.05, 1, 0.94])
        plt.savefig(os.path.join(out_dir, "04_paper_figure.png"), dpi=600)
        plt.savefig(os.path.join(out_dir, "04_paper_figure.pdf"))  # vector version -- use this one in the paper
        plt.close()
